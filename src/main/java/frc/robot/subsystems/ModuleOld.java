 ...
 */
public class ModuleOld extends Module {
   ...
   * @see ModuleConstants
   */
  public static ModuleOld create(ModuleConstants moduleConstants, ShuffleboardTab moduleTab) {
    return new ModuleOld(
  ...
  private ModuleOld(
    ...
    m_steerPIDController.reset(getAngle().getRadians()); // reset the PID, and the Trapezoid motion profile needs to know the starting state
  ...
  public void setAngle(Rotation2d angle) {
    ...
    m_steerPIDOutput = m_steerPIDController.calculate(getAngle().getRadians(), MathUtil.angleModulus(angle.getRadians()));
   ...
   * @return encoder's position from -pi to pi
   */
  public Rotation2d getAngle() {
    return new Rotation2d(MathUtil.angleModulus(m_steerEncoder.getAbsolutePosition() - m_offset));
  }
  ...
  public double getSteerAngleError() {
    return getAngle().minus(m_desiredState.angle).getRadians();
  }
  ...
  public double getDriveVelocityFiltered() {
    return m_driveVelocityMedianFilter.calculate(getState().speedMetersPerSecond);
  }
  ...
  private void setupShuffleboard() {
    ...
    // get drive velocity
    m_moduleTab.addNumber("Vel " + m_moduleType.abbrev + " Raw", () -> getState().speedMetersPerSecond);
    ...
    // Steer angles
    m_moduleTab.addNumber("Angle " + m_moduleType.abbrev, () -> getAngle().getRadians());
    // Steer Velocity
