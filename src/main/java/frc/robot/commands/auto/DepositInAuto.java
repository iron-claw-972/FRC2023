...
public class DepositInAuto extends SequentialCommandGroup {
  ...
  private void run(Position depositPosition, NodeType type, Drivetrain drive, Elevator elevator, FourBarArm arm, Intake intake) {
    addCommands(
  ...
  public DepositInAuto(Position depositPosition, NodeType type, Drivetrain drive, Elevator elevator, FourBarArm arm, Intake intake) {
    ...
    run(depositPosition, type, drive, elevator, arm, intake);
  }

  /**
   * Deposits a game piece in the position found in the SendableChooser from Shuffleboard.
   */
  public DepositInAuto(SendableChooser<Position> depositSelector, SendableChooser<NodeType> typeSelector, Drivetrain drive, Elevator elevator, FourBarArm arm, Intake intake) {
    addRequirements(drive, elevator, arm, intake);
    addCommands(
      new InstantCommand(() -> {
        Position depositPosition = depositSelector.getSelected();
        NodeType type = typeSelector.getSelected();
        run(depositPosition, type, drive, elevator, arm, intake);
      })
      // maybe add wait??
    );
